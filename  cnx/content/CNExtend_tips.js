...
var CNExtend_tips = new function () {
	...
	this.createTip = function(page, name)
	{
		...
		newTipContainer.slowHide = function()
		{
        	...
        	this.timer = setTimeout(function () {currentShowingTip.hide()}, 100);
		}
		...
		newTipContainer.addEventListener("mouseover", function(aEvent) 
			{
				currentShowingTip.show();
			}, false);
		newTipContainer.addEventListener("mouseout", function() 
			{
				currentShowingTip.slowHide();
			}, false);
		
		child(newTipContainer, "tipheader").textContent = "CNx Estimate";
