...
var CNExtend_main = new function () {
	...
	function onPageLoad(aEvent)
	{
		...
		try {	
			...
			if (!(CNExtendIsEnabled()) || !(CNExtend_main.atCN(currentPage)))
			{
				return;
			}
			...
			CNExtend_util.error(exception, CNExtend_enum.errorType.Unknown, false);
	...
	function processNationView(page)
	{
		...
		{
			if (exception instanceof CNExtend_exception.ValidationError)
			{
				CNExtend_util.error(exception,CNExtend_enum.errorType.Validation, true);
			}
			else
			{
				throw exception;
	...
	function processWonderView(page)
	{		
	...
	function setupLayoutPopupMenus() {
		...
		if (layoutDirPath)
		{			
			CNExtend_util.mapTaggedElements('cnextend-layouts-popup', function(me) {me.ref=layoutDirPath;});
		}
	}
	...
	this.enableCNExtend = function()
	{
		CNExtend_util.PrefObserver.setBoolPreference(CNExtend_enum.IS_ENABLED_PREF, true);
	}

	this.disableCNExtend = function()
	{
		CNExtend_util.PrefObserver.setBoolPreference(CNExtend_enum.IS_ENABLED_PREF, false);
	}
	
	this.setLayoutPath = function(pathToSet)
	{
		CNExtend_util.PrefObserver.setStringPreference(CNExtend_enum.SELF_LAYOUT_PATH_PREF, pathToSet);
	}
	
	this.selfLayoutPath = function()
	{
		return CNExtend_util.PrefObserver.getStringPreference(CNExtend_enum.SELF_LAYOUT_PATH_PREF);
	}
	...
	{
		CNExtend_util.newTabSite(CNExtend_enum.website + CNExtend_enum.helpPage)
	}
